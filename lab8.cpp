...
public:
   ...
   //function for range searching the given valus
   int range_search(int k,int l)
   {
       //initialising the variable which will store no f value in between
       int c=0;
       //if both value are present
       if(search(k) && search(l) != NULL)
       {
           //helper function
            linedisplay(root,k,l,c);
            cout<<"REACHED "<<endl;
            cout<<"Total no of nodes in between are "<<c<<endl;
       }
       //if values not present
       else
       {
           cout<<"The values are not found tree"<<endl;
       }
       return 0;
   }
   //helper function
   int linedisplay(Node * current,int k, int l , int &c)
   {
       //if left of current is greater than k
       if((current->left != NULL) && current->left->data >= k){
             //recursion
             linedisplay(current->left,k,l,c);}
       //printing current's dara and increasing c
       cout<<current->data<<",";++c;

       //going to current's right if it is smaller than k
       if((current->right != NULL) && current->right->data <= l){
             //recursion
             linedisplay(current->right,k,l,c);}
